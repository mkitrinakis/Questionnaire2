...
namespace Questionnaire2.Models
{
    public class QuestionnaireModel
    {
        ...
        public Dictionary<int, string> Q22Options = InMemory.getGoodScale(); 
        ...
        public List<string> validateNotFilled()
        {
            ...
            for (int i=1; i<=22; i++)
            {
               string val = (string) typeof(QuestionnaireModel).GetProperty("Q" + i).GetValue(this);
                vals.Add(i, val);
                if (i.Equals(8) && !vals[7].Equals("1")) continue; 
                if ((val ?? "").Trim().Equals("")) { rs.Add("Η ερώτηση (" + i + ") δεν έχει συμπληρωθεί"); }
                if (i.Equals(8) || i.Equals(9))
                {
                    if (val.Equals("5") )
                    {
                        string other = (string)typeof(QuestionnaireModel).GetProperty("Q" + i + "OptionsOther").GetValue(this);
                        if ((other ?? "").Trim().Equals("")) { { rs.Add("Η ερώτηση (" + i + ") πρέπει να συμπληρωθεί η επιλογή 'Αλλο' "); } }
                    }
                }
            }
